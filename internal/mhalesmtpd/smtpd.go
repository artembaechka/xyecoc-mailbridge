// Package mhalesmtpd is a fork of github.com/mhale/smtpd v0.8.3 with small fixes
// for interoperability (DATA terminator, final reply text).
package mhalesmtpd
...
// Results in a "250 2.0.0 Ok" response.
...
// Results in a "250 2.0.0 Ok <message-id>" response when id is non-empty.
...
func (srv *Server) ConfigureTLSWithPassphrase(
	...
	if keyDERBlock == nil {
		return fmt.Errorf("invalid PEM in key file")
	}
		...
		case "XCLIENT":
			...
			if s.xClientTrust {
				...
				for _, xCArg := range xCArgs {
					xCArg = strings.TrimSpace(xCArg)
					if xCArg == "" {
						continue
					}
					// SplitN so a value may contain '='; require KEY=value (at least one '=').
					parts := strings.SplitN(xCArg, "=", 2)
					if len(parts) < 2 {
						continue
					}
					key := strings.ToUpper(strings.TrimSpace(parts[0]))
					val := strings.TrimSpace(parts[1])
					if key == "ADDR" && net.ParseIP(val) != nil {
						s.xClientADDR = val
					}
					if key == "NAME" && len(val) > 0 && val != "[UNAVAILABLE]" {
						s.xClientNAME = val
					}
				}
...
func (s *session) readData() ([]byte, error) {
	...
	for {
		...
		// End of data: RFC 5321 recommends \r\n.\r\n; some clients send \n.\n.
